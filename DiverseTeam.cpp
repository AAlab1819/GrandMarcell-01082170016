...
int main()            //Main program
{
    ...
    int numberofstudents;             //Declare integer for student amount
    int teamSize;                       //Declare integer for team size
    int validmembercount = 0;           //Declare integer for amount of valid members to be placed on a team (all must have unique rating)

    cin >> numberofstudents >> teamSize;          //Input number of students and size of team
    int studentrating[numberofstudents];                 //Declare array for student rating - size depends on number of students
    for (int a = 0; a < numberofstudents; a++)
    {
        cin >> studentrating[a];           //Looping - the user inputs each individual student's rating (loop length depends on the number of students
    }

    for (int p = 0; p < numberofstudents-1; p++)          //Looping - determining whether there are duplicates within the rating array
    {
        for (int q = p+1; q < numberofstudents; q++)
        {
    ...
    for (int x = 0; x < numberofstudents; x++)            //Looping - to push qualifying member counts into the stack
    {

        if (studentrating[x] != 0 )            //If the current rating is not a duplicate
        {
